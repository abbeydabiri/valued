...
func (this *Referral) Process(httpRes http.ResponseWriter, httpReq *http.Request, curdb database.Database) {
	...
	case "":
		...
		this.pageMap["referral-search"] = searchResult

		contentHTML := strconv.Quote(string(this.Generate(this.pageMap, nil)))
		httpRes.Write([]byte(`{"pageTitle":"Search Referrals","mainpanelContentSearch":` + contentHTML + `}`))
		return
...
func (this *Referral) quicksearch(httpRes http.ResponseWriter, httpReq *http.Request, curdb database.Database) {

	tblReferral := new(database.Referral)
	...
	xDocresult := tblReferral.Search(xDocrequest, curdb)
...
func (this *Referral) search(httpReq *http.Request, curdb database.Database) (formSearch, searchPagination map[string]interface{}) {
	...
	xDocPagination := tblReferral.Search(xDocrequest, curdb)
	...
	for cNumber, xDoc := range xDocresult {
		xDoc := xDoc.(map[string]interface{})
		xDoc["number"] = cNumber
		formSearch[cNumber+"#referral-search-result"] = xDoc
...
func (this *Referral) new(httpReq *http.Request, curdb database.Database) string {
	formNew := make(map[string]interface{})

	formSelection := make(map[string]interface{})
	formSelection["formtitle"] = "Add"
	formNew["referral-edit"] = formSelection
	return strconv.Quote(string(this.Generate(formNew, nil)))
}

func (this *Referral) save(httpRes http.ResponseWriter, httpReq *http.Request, curdb database.Database) {
	...
	if html.EscapeString(httpReq.FormValue("title")) == "" {
		sMessage += "Referral Name is missing <br>"
	}
	...
	if html.EscapeString(httpReq.FormValue("control")) != "" {
		...
		tblReferral.Update(this.mapCache["username"].(string), xDoc, curdb)
	} else {
		xDoc["control"] = tblReferral.Create(this.mapCache["username"].(string), xDoc, curdb)
	}

	viewHTML := this.view(xDoc["control"].(string), curdb)
	httpRes.Write([]byte(`{"error":"Referral <b>` + xDoc["title"].(string) + `</b> Saved","mainpanelContent":` + viewHTML + `}`))
}

func (this *Referral) view(control string, curdb database.Database) string {
	...
	ResMap := tblReferral.Read(xDocRequest, curdb)
	if ResMap["1"] == nil {
		return ""
	}
	...
	formView["referral-view"] = xDocRequest

	return strconv.Quote(string(this.Generate(formView, nil)))
}

func (this *Referral) edit(httpRes http.ResponseWriter, httpReq *http.Request, curdb database.Database) {
	...
	formView["referral-edit"] = xDocRequest

	viewHTML := strconv.Quote(string(this.Generate(formView, nil)))
	httpRes.Write([]byte(`{"mainpanelContent":` + viewHTML + `}`))
}

func (this *Referral) activate(httpRes http.ResponseWriter, httpReq *http.Request, curdb database.Database) {

	if html.EscapeString(httpReq.FormValue("control")) == "" {
		httpRes.Write([]byte(`{"error":"Please select a Referral"}`))
		return
	}

	curdb.Query(fmt.Sprintf(`update referral set workflow = 'active' where control = '%s'`,
		html.EscapeString(httpReq.FormValue("control"))))

	httpRes.Write([]byte(`{"triggerSearch":true}`))
}

func (this *Referral) activateView(httpRes http.ResponseWriter, httpReq *http.Request, curdb database.Database) {
...
func (this *Referral) deactivate(httpRes http.ResponseWriter, httpReq *http.Request, curdb database.Database) {
	...
	curdb.Query(fmt.Sprintf(`update referral set workflow = 'inactive' where control = '%s'`,
		html.EscapeString(httpReq.FormValue("control"))))

	httpRes.Write([]byte(`{"triggerSearch":true}`))
}

func (this *Referral) deactivateView(httpRes http.ResponseWriter, httpReq *http.Request, curdb database.Database) {
	...
	viewHTML := this.view(html.EscapeString(httpReq.FormValue("control")), curdb)
	httpRes.Write([]byte(`{"error":"Referral De-Activated","mainpanelContent":` + viewHTML + `}`))
}

func (this *Referral) deactivateAll(httpRes http.ResponseWriter, httpReq *http.Request, curdb database.Database) {
	...
	sqlQuery := fmt.Sprintf(`update referral set workflow = 'inactive' where control in ('0'%s)`, controlList)
